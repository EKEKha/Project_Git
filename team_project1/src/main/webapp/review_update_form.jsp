...
onload = function() {                                             // 페이지가 로드되면 자동으로 실행
	              let frm = document.form(0);                    // 처음 폼을 frm으로 정의
	              let rev_score = ${article.rev_score}
	
	for(int i=0;i<4;i++){
		if( rev_score == frm.rev_score[i]) {
			frm.rev_score[i].checked = true;}          // 배열로 표시가능
	} 
}
</script>
	...
	<form class="mb-3" name="myform" id="myform" method="post" action="revUpdatePro.do?board_no=${article.board_no}" enctype="multipart/form-data">	
            ...
            <div class="fileContainer">
                <div class="fileInput">	
                    <p>FILE NAME: </p>
                    <p id="fileName">${article.rev_fileName}</p>
                </div>
