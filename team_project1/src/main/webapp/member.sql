...
update member set mem_grade='1' where mem_id='loveing30';
update member set mem_point=1000  where mem_id='loveing30';
select * from member;
...
select mem_id,mem_grade,ifnull(o_status,'없음') as o_status,ifnull(sum(totalPrice),0) as totalPrice
...
/*회원별 구매총금액 (관리자제외) :-처리완료 조건  X*/
select mem_id,mem_grade,mem_point,ifnull(sum(totalPrice),0) as totalPrice
from member left join order_item
using(mem_id)
